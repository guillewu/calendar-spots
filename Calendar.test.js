...
describe('getAvailableSpot', function () {
  it('Should get 1 available spots of calendar 4', function () {
    ...
    assert.equal(
      ...
      moment.utc('2023-04-10T16:05:00.000Z').valueOf()
    )
    assert.equal(
      result[0].clientEndHour.valueOf(),
      moment.utc('2023-04-10T16:35:00.000Z').valueOf()
    )
    assert.equal(
      result[0].endHour.valueOf(),
      moment.utc('2023-04-10T16:45:00.000Z').valueOf()
    )

    assert.equal(
      result[1].startHour.valueOf(),
      moment.utc('2023-04-10T19:10:00.000Z').valueOf()
    )
    assert.equal(
      result[1].clientStartHour.valueOf(),
      moment.utc('2023-04-10T19:15:00.000Z').valueOf()
    )
    assert.equal(
      result[1].clientEndHour.valueOf(),
      moment.utc('2023-04-10T19:45:00.000Z').valueOf()
    )
    assert.equal(
      result[1].endHour.valueOf(),
      moment.utc('2023-04-10T19:55:00.000Z').valueOf()
    )
  })
})
